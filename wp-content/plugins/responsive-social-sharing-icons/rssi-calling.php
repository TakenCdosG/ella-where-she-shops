...
function rssi_social_sharing_icons()
{
   ...
   $rssi_content .= '<ul class="rssi-icons clearfix" style="width:225px;">';
	...
	if($options['show_twitter']) {			
		$rssi_content .= '<li >
					<a href="http://twitter.com/home?status=' . urlencode(get_the_title() )  . ' - ' . urlencode(wp_get_shortlink() ). '" style="background-color:white; float:left; padding:0px;  "  target="_blank">
	...
	if($options['show_fb']) {
		$rssi_content .= '<li >
						<a href="https://www.facebook.com/sharer/sharer.php?u=' .urlencode(get_permalink() ) . ' " style="background-color:white; float:left; padding:0px 0px 0px 2px;  margin:0px 0px 0px 0px;" target="_blank">
