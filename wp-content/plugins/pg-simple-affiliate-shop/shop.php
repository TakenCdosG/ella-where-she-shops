 ...
 */
	...
	/* TODO: Check to see if this code is needed and activate it if so
		...
		if ($shop_query->have_posts()) {
				...
				while($shop_query->have_posts()) : $shop_query->the_post();
					...
					if ($this->opt_shop_image_clickable)	
						$output .= '<a " href="'.$this->get_metavalue($post_id, 'link').'" target="_blank" rel="nofollow">'."\n";;
					$output .= '<img class="alignleft pg-sas-image" src="'. $this->get_metavalue($post_id, 'image'). '" alt="'. get_the_title(). '" >'."\n";
					if ($this->opt_shop_image_clickable)
						$output .= '</a>'."\n";

					$output .= ($showtitle == 'true')? '	<div class="whats_new_title"><a href="'.$this->get_metavalue($post_id, 'link').'" >' . get_the_title() .'</a></div>'."\n" :'';
					$output .= '<div class="whats_new_description">'."\n";
		...
		if ($shop_query->have_posts()) {
				...
				$post_id = get_the_ID();
					...
					if ( $this->opt_banner_button_show )
						$output .= '<a class="pg-sas-banner-button pg-sas-buy '. $this->opt_button_color .' small" href="'. $this->get_metavalue($post_id, 'link'). '" title="'. get_the_title(). '" target="_blank" rel="nofollow">'.$this->opt_banner_button_text.'</a>';	
					$output .= '<a class="pg-sas-banner pg-sas-banner-img-link" href="'. $this->get_metavalue($post_id, 'link'). '" title="'. get_the_title(). '" target="_blank" rel="nofollow">';	
	...
	function shortcode_image( $atts, $content = null, $tag ){
		...
		if ($shop_query->have_posts() ) {
				...
				while($shop_query->have_posts()) : $shop_query->the_post();
					...
					$output .= '<a href="'. $this->get_metavalue($post_id, 'link'). '" target="_blank" rel="nofollow" '.$title.' >';
