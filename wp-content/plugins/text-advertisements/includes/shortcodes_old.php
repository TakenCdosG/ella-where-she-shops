...
function shortcodes_text_advertisements() {
	...
	if ($the_query->have_posts()) {
		...
		while ( $the_query->have_posts() ) {
			...
			$out .= '
			<a href="http://' . $textads_link . '/" title="' . get_the_title() . '" class="' . get_the_title() . '"  style="text-decoration:none; color:#566422; font-size:9pt;">' . get_the_title() . '</a>, 
			';
		}
		$out .= '<span style="background-color:white; margin-left:-8px;">&nbsp;</span></div>';
	} else {
		$out .= '
