...
/***************************************************************************/
	...
	function themify_slider($data=array()){
		...
		$speed_options = array(__('Slow', 'themify') => 4000, __('Normal', 'themify') => 3000, __('Fast', 'themify') => 2000);
		...
		$output .= '</div>
				...
				<span class="label">' . __('Auto Play', 'themify') . '</span>
						...
						foreach($auto_options as $option){
							if($option == $data['setting-slider_auto']){
	...
	function themify_slider_action($data=array()){
		...
		jQuery(document).ready(function($){
			$('#slider').flexslider({
				...
				slideshow: <?php if($auto == "yes") echo "true"; else echo "false"; ?>,
				animationLoop: true, 
				directionNav: true,
				prevText: "&laquo;",
				nextText: "&raquo;",
